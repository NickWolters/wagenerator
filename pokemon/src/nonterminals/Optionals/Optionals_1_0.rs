...
#[derive(Debug)]
pub(crate) struct Optionals_1_0;
impl<'a> wagon_gll::Label<'a> for Optionals_1_0 {
    ...
    > {
        Ok(vec![(vec![state.get_label_by_uuid("Shiny") ?], None),])
    }
    fn is_eps(&self) -> bool {
        false
    }
    ...
    fn code(&self, state: &mut wagon_gll::GLLState<'a>) -> wagon_gll::GLLResult<'a, ()> {
        ...
        let label = state.get_label(&wagon_ident::Ident::Unknown("Shiny".to_string()));
        state
            .gss_pointer = state
            .create(
                &std::rc::Rc::new(
                ...
                vec![
                    s_shiny.clone(), s_d_ability, s_d_shiny, s_d_level, s_d_happy,
                    s_d_nat, s_d_ev, s_d_iv, s_shiny,
                ],
            )?;
        label.code(state)
    }
    ...
    fn _weight(
        ...
        fn actual_weight<'a>(
            ...
            let s_d_shiny = state.get_attribute(1usize)?.to_owned();
            Ok(wagon_gll::value::Value::from(std::ops::Not::not(s_d_shiny.clone())?))
        }
