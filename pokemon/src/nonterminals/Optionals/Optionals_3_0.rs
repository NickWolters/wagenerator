...
#[derive(Debug)]
pub(crate) struct Optionals_3_0;
impl<'a> wagon_gll::Label<'a> for Optionals_3_0 {
    ...
    > {
        Ok(vec![(vec![state.get_label_by_uuid("Happiness") ?], None),])
    }
    fn is_eps(&self) -> bool {
        false
    }
    ...
    fn code(&self, state: &mut wagon_gll::GLLState<'a>) -> wagon_gll::GLLResult<'a, ()> {
        ...
        let label = state
            .get_label(&wagon_ident::Ident::Unknown("Happiness".to_string()));
        state
            .gss_pointer = state
            .create(
                &std::rc::Rc::new(
    ...
    fn _weight(
        ...
        fn actual_weight<'a>(
            ...
            let s_d_happy = state.get_attribute(3usize)?.to_owned();
            Ok(wagon_gll::value::Value::from(std::ops::Not::not(s_d_happy.clone())?))
        }
