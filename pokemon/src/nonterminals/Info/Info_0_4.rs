#[allow(non_camel_case_types)]
#[derive(Debug)]
pub(crate) struct Info_0_4;
impl<'a> wagon_gll::Label<'a> for Info_0_4 {
    #[allow(unused_variables)]
    fn first_set(
        &self,
        state: &wagon_gll::GLLState<'a>,
    ) -> wagon_gll::ImplementationResult<
        'a,
        Vec<(Vec<wagon_gll::GLLBlockLabel<'a>>, Option<wagon_gll::Terminal<'a>>)>,
    > {
        Ok(vec![(vec![], Some(& [])),])
    }
    fn is_eps(&self) -> bool {
        true
    }
    fn uuid(&self) -> &str {
        "Info_0_4"
    }
    #[allow(unused_variables)]
    fn code(&self, state: &mut wagon_gll::GLLState<'a>) -> wagon_gll::GLLResult<'a, ()> {
        let s_item = if let Some(v) = state.get_ret_val(0usize)? {
            v
        } else {
            state.restore_attribute(3usize)?
        }
            .to_owned();
        let s_nickname = state.restore_attribute(1usize)?.clone();
        let s_gender = state.restore_attribute(2usize)?.clone();
        Ok(
            state
                .pop(
                    &vec![
                        Some(s_nickname.clone()), Some(s_gender.clone()), Some(s_item
                        .clone()),
                    ],
                    vec![s_nickname, s_gender, s_item,],
                )?,
        )
    }
    fn to_string(&self) -> &str {
        ""
    }
    fn str_parts(&self) -> Vec<&str> {
        vec![]
    }
    fn attr_rep_map(&self) -> (Vec<&str>, Vec<&str>) {
        (vec!["&item",], vec!["&nickname", "&gender", "&item",])
    }
    #[allow(unused_variables)]
    fn _weight(
        &self,
        state: &wagon_gll::GLLState<'a>,
    ) -> Option<wagon_gll::ImplementationResult<'a, wagon_gll::value::Value<'a>>> {
        Some(
            Err(
                wagon_gll::GLLImplementationError::Fatal(
                    "Weight should never be evaluated for non-zero GLL blocks",
                ),
            ),
        )
    }
}
