#[path = "LevelGuard/LevelGuard_0_0.rs"]
pub(crate) mod LevelGuard_0_0;
#[allow(non_camel_case_types)]
#[derive(Debug)]
pub(crate) struct LevelGuard;
impl<'a> wagon_gll::Label<'a> for LevelGuard {
    ...
    > {
        Ok(vec![(vec![state.get_label_by_uuid("LevelGuard_0_0") ?], None),])
    }
    fn is_eps(&self) -> bool {
        false
    }
    ...
    fn code(&self, state: &mut wagon_gll::GLLState<'a>) -> wagon_gll::GLLResult<'a, ()> {
        ...
        let fst = state.get_label_by_uuid("LevelGuard_0_0")?;
        if state.test_next(&fst)? {
            let root = state.get_label_by_uuid("LevelGuard")?;
            let rules = state.get_rule("LevelGuard_0")?;
            let slot = wagon_gll::GrammarSlot::new(root, rules, 0, 0, "LevelGuard_0_0");
