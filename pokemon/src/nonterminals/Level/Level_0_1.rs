...
#[derive(Debug)]
pub(crate) struct Level_0_1;
impl<'a> wagon_gll::Label<'a> for Level_0_1 {
    ...
    fn code(&self, state: &mut wagon_gll::GLLState<'a>) -> wagon_gll::GLLResult<'a, ()> {
        let l_total = if let Some(v) = state.get_ret_val(0usize)? {
            ...
            .to_owned();
        let label = state
            .get_label(&wagon_ident::Ident::Unknown("LevelGuard".to_string()));
                ...
                .create(
                    &std::rc::Rc::new(
                        wagon_gll::GrammarSlot::new(
                            state.get_label_by_uuid("Level")?,
                            state.get_rule("Level_0")?,
                            2usize,
                            0,
                            "Level_0",
                        ),
