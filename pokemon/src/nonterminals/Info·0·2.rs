#[path = "Info·0·2/Info·0·2_0_0.rs"]
pub(crate) mod Info·0·2_0_0;
#[path = "Info·0·2/Info·0·2_0_1.rs"]
pub(crate) mod Info·0·2_0_1;
#[path = "Info·0·2/Info·0·2_1_0.rs"]
pub(crate) mod Info·0·2_1_0;
#[allow(non_camel_case_types)]
#[derive(Debug)]
pub(crate) struct Info·0·2;
impl<'a> wagon_gll::Label<'a> for Info·0·2 {
    ...
    > {
        Ok(
            vec![
                (vec![state.get_label_by_uuid("Info·0·2_0_0") ?, state
                .get_label_by_uuid("Info·0·2_0_1") ?], None), (vec![state
                .get_label_by_uuid("Info·0·2_1_0") ?], None),
            ],
        )
    }
    fn is_eps(&self) -> bool {
    ...
    fn code(&self, state: &mut wagon_gll::GLLState<'a>) -> wagon_gll::GLLResult<'a, ()> {
        ...
        let fst = state.get_label_by_uuid("Info·0·2_0_0")?;
        if state.test_next(&fst)? {
            let root = state.get_label_by_uuid("Info·0·2")?;
            let rules = state.get_rule("Info·0·2_0")?;
            let slot = wagon_gll::GrammarSlot::new(root, rules, 0, 0, "Info·0·2_0_0");
        ...
        let fst = state.get_label_by_uuid("Info·0·2_1_0")?;
        if state.test_next(&fst)? {
            let root = state.get_label_by_uuid("Info·0·2")?;
            let rules = state.get_rule("Info·0·2_1")?;
            let slot = wagon_gll::GrammarSlot::new(root, rules, 0, 0, "Info·0·2_1_0");
            let weight = slot.weight(state)?;
            if wagon_value::Valueable::is_truthy(&weight)? {
                candidates.push((weight, std::rc::Rc::new(slot)));
            } else {
