#[path = "YesNo/YesNo_0_0.rs"]
pub(crate) mod YesNo_0_0;
#[path = "YesNo/YesNo_1_0.rs"]
pub(crate) mod YesNo_1_0;
#[allow(non_camel_case_types)]
#[derive(Debug)]
pub(crate) struct YesNo;
impl<'a> wagon_gll::Label<'a> for YesNo {
    ...
    > {
        Ok(
            vec![
                (vec![state.get_label_by_uuid("YesNo_0_0") ?], None), (vec![state
                .get_label_by_uuid("YesNo_1_0") ?], None),
            ],
        )
    }
    fn is_eps(&self) -> bool {
    ...
    fn code(&self, state: &mut wagon_gll::GLLState<'a>) -> wagon_gll::GLLResult<'a, ()> {
        ...
        let fst = state.get_label_by_uuid("YesNo_0_0")?;
        if state.test_next(&fst)? {
            let root = state.get_label_by_uuid("YesNo")?;
            let rules = state.get_rule("YesNo_0")?;
            let slot = wagon_gll::GrammarSlot::new(root, rules, 0, 0, "YesNo_0_0");
        ...
        let fst = state.get_label_by_uuid("YesNo_1_0")?;
        if state.test_next(&fst)? {
            let root = state.get_label_by_uuid("YesNo")?;
            let rules = state.get_rule("YesNo_1")?;
            let slot = wagon_gll::GrammarSlot::new(root, rules, 0, 0, "YesNo_1_0");
            let weight = slot.weight(state)?;
            if wagon_value::Valueable::is_truthy(&weight)? {
                candidates.push((weight, std::rc::Rc::new(slot)));
            } else {
