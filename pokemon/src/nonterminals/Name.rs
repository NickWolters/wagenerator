#[path = "Name/Name_0_0.rs"]
pub(crate) mod Name_0_0;
#[path = "Name/Name_0_1.rs"]
pub(crate) mod Name_0_1;
#[allow(non_camel_case_types)]
#[derive(Debug)]
pub(crate) struct Name;
impl<'a> wagon_gll::Label<'a> for Name {
    ...
    > {
        Ok(
            vec![
                (vec![state.get_label_by_uuid("Name_0_0") ?, state
                .get_label_by_uuid("Name_0_1") ?], None),
            ],
        )
    }
    fn is_eps(&self) -> bool {
    ...
    fn code(&self, state: &mut wagon_gll::GLLState<'a>) -> wagon_gll::GLLResult<'a, ()> {
        ...
        let fst = state.get_label_by_uuid("Name_0_0")?;
        if state.test_next(&fst)? {
            let root = state.get_label_by_uuid("Name")?;
            let rules = state.get_rule("Name_0")?;
            let slot = wagon_gll::GrammarSlot::new(root, rules, 0, 0, "Name_0_0");
